...
app.get('/orders', async (req, res) => {
    try {
        // Отримання замовлень і продуктів з бази даних
        const orders = await Order.find({});
        const products = await Product.find({});

        // Створення мапи продуктів для швидкого доступу
        const productMap = products.reduce((map, product) => {
            map[product.id] = product;
            return map;
        }, {});

        // Формування детального списку замовлень
        const detailedOrders = orders.map(order => {
            ...
            const productList = Object.entries(cart || {}).flatMap(([productId, items]) => {
                // Перевірка валідності ключа і його наявності у productMap
                if (!productMap[productId]) {
                    console.warn(`Товар з ID ${productId} не знайдено або некоректний`);
                    return [];
                }

                // Перевірка, чи `items` є масивом
                if (!Array.isArray(items)) {
                    console.warn(`Некоректний формат для товарів з ID ${productId}:`, items);
                    return [];
                }

                // Обробка кожного елемента у `items`
                return items.map(item => ({
                    name: productMap[productId]?.name || "Невідомий товар",
                    image: productMap[productId]?.image || "url-to-placeholder",
                    size: item.size,
                    quantity: item.quantity,
                }));
            });

            // Формування детального об'єкта замовлення
            return {
                orderId: _id,
                phone,
                address: `${region}, ${city}, Відділення: ${postOffice}`,
                status,
                paymentStatus: order.paymentStatus || "Невідомо",
...
app.post('/createorder', fetchUsers, async (req, res) => {
    ...
    try {
        ...
        const paymentData = {
            ...
            server_callback_url: `https://b16b-91-237-40-7.ngrok-free.app/paymentcallback`,
